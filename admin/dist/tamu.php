...
if(isset($_GET['aksi'])){
    if($_GET['aksi']=="edit"){
        $query = "SELECT * FROM tamu WHERE id_tamu='$_GET[id]'";
        $edit = mysqli_query($koneksi,$query);
        while($data=mysqli_fetch_array($edit)){
            $nik_admin = $data['nik_tamu'];
            $nama_admin = $data['nama_tamu'];
    ...
    if ($op == 'edit') { 
        $id = $_GET['id'];
        $sql1 = "select * from tamu where id_tamu = '$id'";
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Data Tamu</li>
                                    ...
                                    <form action="edittamu.php" method="POST" enctype="multipart/form-data">
                                        <div class="mb-3 row">
                                            <label for="nim" class="col-sm-2 col-form-label">Nik</label>
                                            <div class="col-sm-10">
                                            <input type="hidden" name="id" value="<?php echo $id?>" />
                                            <input type="text" class="form-control" id="nik" name="nik_tamu" value="<?php echo $nik ?>" />
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                                            <label for="nama" class="col-sm-2 col-form-label">Nama</label>
                                            <div class="col-sm-10">
                                            <input type="text" class="form-control" id="nama" name="nama_tamu" value="<?php echo $nama ?>" />
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                            ...
                            <div class="card-body">
                                <table id="datatablesSimple">
                                    ...
                                    <?php
                                        ...
                                        <TR>
                                            <td><?=$urut;$urut++?></td>
                                            <td><?=$admin['nik_tamu']?></td>
                                            <td><?=$admin['nama_tamu']?></td>
                                            <td><?=$admin['username']?></td>
                                            <td><?=$admin['password']?></td>
                                            <td scope="row">
                                                <a href="tamu.php?op=edit&id=<?php echo $id ?>"><button type="button"
                                                    class="btn btn-warning">Edit</button></a>
                                                <a href="edittamu.php?op=delete&id=<?php echo $id ?>"> <button type="button" class="btn btn-danger"
