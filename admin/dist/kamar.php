...
$mysql_adm=mysqli_query($koneksi, "select * from kamar where username='$_SESSION[username]'");


if(isset($_GET['aksi'])){
    if($_GET['aksi']=="edit"){
        $query = "SELECT * FROM kamar WHERE id_kamar='$_GET[id]'";
        $edit = mysqli_query($koneksi,$query);
        while($data=mysqli_fetch_array($edit)){
            $nik_admin = $data['no_kamar'];
            $nama_admin = $data['type_kamar'];
            $status = $data['status'];
            $foto = $data['foto'];
        }
    }
                    ...
                    <div class="container-fluid px-4">
                        ...
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item active">Data Kamar</li>
                                    ...
                                    <form action="editkamar.php" method="POST" enctype="multipart/form-data">
                                        <div class="mb-3 row">
                                            <label for="nim" class="col-sm-2 col-form-label">No</label>
                                            <div class="col-sm-10">
                                            <input type="hidden" name="id" value="<?php echo $id?>" />
                                            <input type="text" class="form-control" id="nik" name="no_kamar" value="<?php echo $nik ?>" />
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                                            <label for="nama" class="col-sm-2 col-form-label">Nama</label>
                                            <div class="col-sm-10">
                                            <input type="text" class="form-control" id="nama" name="type_kamar" value="<?php echo $nama ?>" />
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                            ...
                            <div class="card-body">
                                <table id="datatablesSimple">
                                    ...
                                    <?php
                                        ...
                                        <TR>
                                            <td><?=$urut;$urut++?></td>
                                            <td><?=$admin['no_kamar']?></td>
                                            <td><?=$admin['type_kamar']?></td>
                                            <td><?=$admin['harga']?></td>
                                            <td><img src="img/<?=$admin['foto']?>" width="30%" height="30%" ></td>
                                            <td><?=$admin['status']?></td>
                                            <td scope="row">
                                                <a href="kamar.php?op=edit&id=<?php echo $id ?>"><button type="button"
                                                    class="btn btn-warning">Edit</button></a>
                                                <a href="editkamar.php?op=delete&id=<?php echo $id ?>"> <button type="button" class="btn btn-danger"
